		...
		snow: function(){
			var xhr = {
				trigger: function(){
					...
					console.log(JSON.parse(localStorage['localData']).timestamp)
					console.log(new Date().getTime(), JSON.parse(localStorage['localData']).timestamp)
					
					if( (new Date().getTime() - JSON.parse(localStorage['localData']).timestamp < 60000 ) ) {
						var localDataString = JSON.parse(localStorage.getItem('localData'));
						
						Transparency.render(document.getElementById('renderSnow'), localDataString.data);
						alert('LocalData loaded');
